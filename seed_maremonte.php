<?php
/**
 * Hotel Mare & Monte Bistro Menu Seeder & Database Migration
 */
...
try {
    ...
    // 1. Tabloları oluştur / varsa sütunları güncelle
    ...
    // Drop old tables to do a completely fresh, clean seed for Mare & Monte Bistro
    $pdo->exec("DROP TABLE IF EXISTS product_options");
    $pdo->exec("DROP TABLE IF EXISTS order_items");
    $pdo->exec("DROP TABLE IF EXISTS orders");
    $pdo->exec("DROP TABLE IF EXISTS waiter_calls");
    ...
    $pdo->exec("CREATE TABLE waiter_calls (
        id $autoInc,
    ...
    // 9. Ürün Seçenekleri Tablosu
    $pdo->exec("CREATE TABLE product_options (
        id $autoInc,
        product_id INT NOT NULL,
    ...
    // 11. Müşteri Değerlendirmeleri Tablosu
    ...
    echo "Tablolar sıfırdan oluşturuldu.\n";

    // 1. Admin Kullanıcısı
    $stmtAdmin = $pdo->prepare("INSERT INTO admins (username, password_hash, name, role) VALUES (?, ?, ?, ?)");
    $stmtAdmin->execute(['admin', password_hash('admin123', PASSWORD_DEFAULT), 'Mare & Monte Admin', 'superadmin']);
    ...
    $settings = [
        ...
        // MODÜL AÇ/KAPA
        'enable_order' => '0',            // Masadan Doğrudan Sipariş Pasif
        'enable_multi_lang' => '1',       // Çoklu Dil Açık
        'enable_kitchen' => '1',          // Mutfak Ekranı Açık
        'enable_stories' => '1',          // Hikayeler Açık
        'enable_popup' => '1',            // Pop-Up Kampanya Açık
        'enable_feedback' => '1',         // Puanlama Açık
        'enable_allergens_filter' => '1', // Diyet / Alerjen Filtresi Açık
        'enable_waiter_call' => '1',      // Garson Çağırma & Hesap Açık
    ...
    echo "Başarıyla $totalCats kategori ve $totalProds ürün eklendi.\n";

    // 6. Örnek Yorumlar
    $feedbacks = [
        ['table_number' => '4', 'rating' => 5, 'name' => 'Ahmet Kaya', 'comment' => 'Bonfile lokum ve karides güveç tek kelimeyle muhteşemdi! Altınoluk\'un en iyi lezzeti.'],
        ['table_number' => 'I2', 'rating' => 5, 'name' => 'Selin Yılmaz', 'comment' => 'İskelede gün batımı eşliğinde taze levrek ve kalamar harikaydı, servis çok hızlı.'],
        ['table_number' => 'B1', 'rating' => 5, 'name' => 'Mehmet Öz', 'comment' => 'Mare Burger ve pizzalar çok lezzetli, çocuklar da bayıldı. Teşekkürler.']
    ];
    $stmtFb = $pdo->prepare("INSERT INTO feedback (table_number, rating, name, comment, is_read) VALUES (?, ?, ?, ?, 1)");
    foreach ($feedbacks as $fb) {
        $stmtFb->execute([$fb['table_number'], $fb['rating'], $fb['name'], $fb['comment']]);
    }

    echo "Örnek yorumlar eklendi.\n";
    echo "TAMAMLANDI: Hotel Mare & Monte Bistro menüsü başarıyla kuruldu!\n";
